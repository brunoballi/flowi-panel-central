...
//
// Compartido entre dos disparadores:
//  - app/api/cron/sync-subscriptions (GitHub Actions, cada 1 hora, service role)
//  - app/api/tenants/sync (botón "Actualizar ahora" del dashboard, sesión del usuario)
//
...
export async function syncAllTenants(db: SupabaseClient<any>): Promise<SyncResult> {
  ...
  return { ok: true, synced, failed, total: (tenants ?? []).length }
}
